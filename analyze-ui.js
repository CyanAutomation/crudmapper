...
const designRules = {
  noLineRule: !cssContent.includes('border: 1px solid') || cssContent.includes('border-bottom: 1px'),
  alternatingRows: cssContent.includes('nth-child(odd)') && cssContent.includes('nth-child(even)'),
  activeStateIndicator: cssContent.includes('border-left-color') || cssContent.includes('border-left:'),
...
const jsIntegration = {
  ...
  drageDropHandlers: fs.readFileSync(path.join(process.cwd(), 'main.ts'), 'utf-8').includes('dragenter'),
  fileUploadHandlers: fs.readFileSync(path.join(process.cwd(), 'main.ts'), 'utf-8').includes('addEventListener'),
};
