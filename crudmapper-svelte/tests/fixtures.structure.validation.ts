...
describe("fixtures structure", () => {
  ...
  it("should handle partial role entries", () => {
    ...
    if ("roles" in partialExtracted) {
      ...
      assert.equal(
        partialRole.FriendlyName,
        "PartialRole",
        "Missing FriendlyName should fallback to Name"
      );
  ...
  it("should handle researcher with metadata", () => {
    ...
    if ("roles" in withMetadataExtracted) {
      ...
      assert.equal(
        withMetadataExtracted.roles.length,
        3,
        "Expected three roles in researcher-with-metadata fixture"
      );

      const roles = withMetadataExtracted.roles as unknown[];
      const [metadataReadOnlyRole, metadataUserRole, metadataShadowRole] = roles.map((role) =>
        normalizeRole(role)
      );
      ...
      assert.equal(
        ...
        "Happy-path FriendlyName should be preserved"
      );
      assert.ok(
      ...
      assert.equal(
        metadataUserRole.FriendlyName,
        "Researcher (User)",
        "Unknown keys should not change FriendlyName display label"
      );
      assert.ok(
