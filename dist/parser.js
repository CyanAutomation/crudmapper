...
export function normalizeRole(role) {
    ...
    const normalizedArea = typeof roleObj?.Area === "string" && roleObj.Area.trim().length > 0
        ? roleObj.Area
        : "Unassigned";
    const normalizedRank = Number.isFinite(roleObj?.Rank) ? roleObj.Rank : 0;
    const normalizedFriendlyName = typeof roleObj?.FriendlyName === "string" && roleObj.FriendlyName.trim().length > 0
