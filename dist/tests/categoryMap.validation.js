...
describe("categoryMap", () => {
    ...
    for (const fixture of fixtures) {
        it(`should categorize ${fixture.name} as ${fixture.expected}`, () => {
            assert.equal(getCategoryForPermission(fixture.name), fixture.expected, `Unexpected category for ${fixture.note}: ${fixture.name}`);
        });
    }
    it("should tokenize camelCase correctly", () => {
        assert.deepEqual(tokenizePermissionName("CustomerAccount"), ["customer", "account"], "Expected case transition tokenization to split CustomerAccount");
    });
    it("should tokenize slash-separated names", () => {
        assert.deepEqual(tokenizePermissionName("finance/payment"), ["finance", "payment"], "Expected slash tokenization to split finance/payment");
    });
});
//# sourceMappingURL=categoryMap.validation.js.map