...
describe("parser", () => {
    ...
    for (const fixture of fixtures) {
        it(`should parse '${fixture.input}' correctly`, () => {
            ...
            for (const token of fixture.expectedCrud) {
                assert.ok(normalizedSet.has(token), `Expected NormalizedPermissions[${parsed.canonicalName}] to include ${token}`);
            }
        });
    }
    ...
    it("should handle null permissions gracefully", () => {
        ...
        assert.deepEqual(Object.keys(nullPermissionsRole.NormalizedPermissions ?? {}), [], "Expected no normalized permissions for null Permissions");
    });
    it("should skip non-string permission entries", () => {
        ...
        assert.deepEqual([
            ...mixedPermissionsRole.NormalizedPermissions.account,
        ].sort(), ["C", "D", "R", "U"], "Expected account CRUD letters to come only from valid string entries");
    });
    ...
    it("should handle malformed data in UI rendering", () => {
        ...
        assert.doesNotThrow(() => {
            ...
            renderSidebar({ [malformedForUi.Area]: [malformedForUi] }, () => { }, sidebarContainer);
