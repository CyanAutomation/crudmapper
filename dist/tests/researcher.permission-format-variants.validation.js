...
describe("researcher permission format variants", () => {
    it("should normalize permission format variants to canonical key", () => {
        const variantsFixturePath = path.resolve(__dirname, "../data/fixtures/researcher.permission-format-variants.json");
        ...
        assert.deepEqual(Object.keys(variantsNormalizedRole.NormalizedPermissions ?? {}), ["account"], "Expected all account permission variants to normalize to canonical key account");
        const variantsActualCrud = [
            ...variantsNormalizedRole.NormalizedPermissions.account,
        ]
            .sort()
            .join("");
        const expectedCrud = "CDRU".split("").sort().join("");
        assert.equal(variantsActualCrud, expectedCrud, "Expected deduped CRUD letters in NormalizedPermissions.account to equal CDRU");
    });
    it("should skip invalid permission types", () => {
        const invalidTypesFixturePath = path.resolve(__dirname, "../data/fixtures/researcher.invalid-permission-types.json");
...
//# sourceMappingURL=researcher.permission-format-variants.validation.js.map